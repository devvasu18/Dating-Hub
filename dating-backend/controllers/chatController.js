...
export const getRecentChats = async (req, res) => {
  try {
    ...
    // Aggregate to find the last message per partner and unread count
    const pipeline = [
      {
        $match: {
    ...
    // Lookup partner user details from users collection (if exists)
    // returns user object or minimal fallback
    ...
    const final = results.map((r) => {
      ...
      const partner = usersMap.get(pid) || { _id: pid, name: 'Unknown', images: [] };
      return {
        partnerId: pid,
        partner,
